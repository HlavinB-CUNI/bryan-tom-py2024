...
def plot_medal_histogram(df_total):
    ...
    df_total['results'] = df_total.filter(like='2002_total').sum(axis=1)
    df_total['2002_total'] = pd.to_numeric(df_total['2002_total'])
    
    # Sorting by total medals
    df_total_sorted = df_total.sort_values(by='2002_total', ascending=False)
    ...
    plt.bar(top_countries.index, top_countries['2002_total'], color='gold')
    plt.xlabel('Country')
    plt.ylabel('results')
